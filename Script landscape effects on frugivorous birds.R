...
dados<-read.csv("analysis.csv", header=T, sep=",", dec=".")
dados$coords<-as.matrix(cbind(dados$lon,dados$lat))

mix.all<-glm.nb(fr~scale(fc)+scale(ed)+scale(mnnd)+coords, offset(log(dados$count_effort)),
...
par(mfrow=c(2,1))
plot(resid(mod.fc_ed_mnnd.all)~ID_site, dados, pch=19, xlab= "ID site", ylab="Resid (Richness)")

plot(resid(mod.fc_ed_mnnd.all)~year, dados, pch=19, xlab= "Year", ylab= "Resid (Richness)")
...
dados_fun<- dados<-dados[-c(19, 20, 22, 24, 80, 84, 95, 110, 127, 128, 131, 132),]
...
par(mfrow=c(2,1))
plot(resid(mod.fc_ed_mnnd.fric)~ID_site, dados_fun, pch=19, xlab= "ID site", ylab="Resid (SESFRic)")

plot(resid(mod.fc_ed_mnnd.fric)~ year, dados_fun, pch=19, xlab= "Year", ylab= "Resid (SESFRic)")
...
par(mfrow=c(2,1))
plot(resid(mod.fc_ed_mnnd.fdis)~ID_site, dados_fun, pch=19, xlab= "ID site", ylab="Resid (FDis)")

plot(resid(mod.fc_ed_mnnd.fdis)~ year, dados_fun, pch=19, xlab= "Year", ylab= "Resid (FDis)")

plot(mod.fc_ed_mnnd.fdis, pch=19)

##To calculate semivariogram
...
par(mfrow=c(2,1))
plot(resid(mod.fc_ed_mnnd.body)~ID_site, dados, pch=19, xlab= "ID site", ylab="Resid (Body mass)")

plot(resid(mod.fc_ed_mnnd.body)~ year, dados, pch=19, xlab= "Year", ylab= "Resid (Body mass)")
...
par(mfrow=c(2,1))
plot(resid(mod.fc_ed_mnnd.bill)~ID_site, dados, pch=19, xlab= "ID site", ylab="Resid (Bill width)")

plot(resid(mod.fc_ed_mnnd.bill)~ year, dados, pch=19, xlab= "Year", ylab= "Resid (Bill width)")
...
par(mfrow=c(2,1))
plot(resid(mod.fc_ed_mnnd.hand)~ID_site, dados2, pch=19, xlab= "ID site", ylab="Resid (Hand wing)")

plot(resid(mod.fc_ed_mnnd.hand)~ year, dados2, pch=19, xlab= "Year", ylab= "Resid (Hand wing)")
